...
# Default values
cmd.latency = 1
cmd.rate = 1000000 #0,5 mBps
cmd.on_off_rate = 1000000 #0,5 mBps 
...
def SetupTcpConnection(srcNode, dstNode, dstAddr, startTime, stopTime):
  ...
  sink_apps.Stop(ns.core.Seconds(sim_life)) 
...
#SetupTcpConnection(nodes.Get(2), nodes.Get(0), if0if4.GetAddress(0), ns.core.Seconds(20.0), ns.core.Seconds(40.0))

#SetupTcpConnection(nodes.Get(1), nodes.Get(6), if6if4.GetAddress(0), ns.core.Seconds(20.0), ns.core.Seconds(40.0))
...
SetupUdpConnection(nodes.Get(1), nodes.Get(6), if6if4.GetAddress(0),
                   ns.core.Seconds(2.0), ns.core.Seconds(70.0))

SetupUdpConnection(nodes.Get(1), nodes.Get(6), if6if4.GetAddress(0),
                   ns.core.Seconds(120.0), ns.core.Seconds(200.0))


#######################################################################################
# CREATE BULK TCP CLIENT AND SERVER
#
#

def SetupBulkConnection(srcNode, dstNode, dstAddr, startTime, stopTime, totData):
  # Create a TCP sink at dstNode
  packet_sink_helper = ns.applications.PacketSinkHelper("ns3::TcpSocketFactory", 
                          ns.network.InetSocketAddress(ns.network.Ipv4Address.GetAny(), 
                                                       9876))
  ...
  # Create TCP connection from srcNode to dstNode 
  bulk_helper = ns.applications.BulkSendHelper("ns3::TcpSocketFactory", 
                          ns.network.Address(ns.network.InetSocketAddress(dstAddr, 9876)))
 
  bulk_helper.SetAttribute("MaxBytes", ns.core.UintegerValue(totData))
#  bulk_helper.SetAttribute("SendSize", ns.core.UintegerValue(500))

  
  client_apps = bulk_helper.Install(srcNode)
  client_apps.Start(startTime)
  client_apps.Stop(stopTime) 


SetupBulkConnection(nodes.Get(2), nodes.Get(1), if1if4.GetAddress(0), ns.core.Seconds(30.0), ns.core.Seconds(120.0), 10000000)
...
pointToPoint.EnablePcap("d6d4", d6d4.Get(0), True)
pointToPoint.EnablePcap("d2d5", d2d5.Get(0), True)
...
ns.core.Simulator.Stop(ns.core.Seconds(sim_life))
